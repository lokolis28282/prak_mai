(function(){
  ...
  sections.warehouse=[
    ['overview','Обзор склада'],['balance','Баланс'],['receipt','Приход'],['issue','Расход'],
  ...
  function renderWarehouseOverview(){
    ...
    root.replaceChildren(
      renderElement('div',{className:'warehouse-overview-head',children:[
        renderElement('div',{children:[renderElement('p',{className:'eyebrow',text:'Склад'}),renderElement('h2',{text:'Текущий баланс склада'}),renderElement('p',{text:'Нажмите на категорию или тип, чтобы открыть готовую выборку в Балансе.'})]}),
  ...
  async function runGlobalSearch(query){
    ...
    try{
      ...
      const worksShortcut=renderButton({className:'global-search-result',onClick:()=>{closeGlobalSearch();openWorkRegistrySearch(normalized)},children:[
        ...
        renderElement('span',{text:`«${normalized}» в общем реестре работ`}),
      ...
      // Real equipment/delivery/engineer matches are the primary answer. Keep
      // the cross-module UVR shortcut last so a searched S/N cannot look like
      // an equipment hit but unexpectedly navigate to Reports.
      panel.replaceChildren(
        ...(items.length ? items : [renderElement('div', {className: 'global-search-state', text: 'В складе ничего не найдено'})]),
        worksShortcut,
      );
