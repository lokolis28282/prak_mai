...
function renderDaily(rows){document.getElementById('dailyBody').innerHTML=rows.map(x=>`<tr><td>${esc(x.date)}</td><td>${esc(x.report_block)}</td><td>${esc(x.task_number)}</td><td>${esc(x.description)}</td><td>${esc(x.quantity)}</td><td>${esc(x.serial_number)}</td><td>${esc(operationalHistoryText(x.responsible))}</td><td>${esc(operationalHistoryText(x.comment))}</td></tr>`).join('')||'<tr><td class="empty" colspan="8">Нет данных за период</td></tr>'}
...
function openReportsHub(){
 openTask('reports','daily');showDevelopmentPlaceholder(byId('daily'),'Отчёты — в разработке');
}
