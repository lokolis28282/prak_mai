(function () {
  ...
  // The section filter stays client-side (server search covers text); date,
  // text and needs_review are sent to the paginated endpoint.
  ...
  function render() {
    ...
    body.replaceChildren(...pageRows.map(row => {
      ...
      return renderElement('tr', {className: classes, children: [
        renderElement('td', {className: 'uvr-check-col', children: [check]}),
  ...
  // Server-side filters re-fetch; client-only ones (section, status, sort) re-render.
  ['uvrFilterFrom', 'uvrFilterTo', 'uvrReviewOnly'].forEach(id => {
    const input = byId(id);
  ...
  const sectionFilter = byId('uvrFilterSection');
  if (sectionFilter) sectionFilter.onchange = renderFromFirstPage;
  const statusFilter = byId('uvrFilterStatus');
  ...
  // The status filter is client-side (single select), so re-render on change.
  if (statusFilter) statusFilter.onchange = renderFromFirstPage;
