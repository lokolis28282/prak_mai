(function () {
  ...
  const activeReferences = kind => {
    const reportsOwned = state.report_references?.[kind];
    if (Array.isArray(reportsOwned)) return reportsOwned.slice();
  ...
  function applySourceMode(form) {
    ...
    // Current report forms use `.uvr-desc-cell`; keep the older class as a
    // compatibility fallback for any embedded/custom form.
    const descriptionField = form.querySelector('.uvr-desc-cell, .uvr-description-field');
