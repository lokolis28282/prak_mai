...
- Админские права определяются session context/role, не ФИО. На login-странице
  есть отдельный credentialed admin-вход; внутри рабочего UI нет переключателя,
  способного повысить права уже созданной сессии. API-key auth отсутствует.
...
Current source: ODE `0.21.1` с Multi-Warehouse IXcellerate/Solar, отдельным
...
baseline rehearsal сохранены. Windows package closure и CRLF launcher
исправлены в patch-релизе; текущий ZIP —
