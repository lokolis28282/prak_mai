...
class ReportsHandoverTest(unittest.TestCase):
    ...
    def test_shift_report_xlsx_has_two_sheets(self) -> None:
        ...
        # Both tasks in the done sheet, only the unfinished one in handover.
        self.assertEqual(len(done), 4)  # title + header + 2 rows
        self.assertEqual(len(handover), 3)  # title + header + 1 row
