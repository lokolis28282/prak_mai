# ODE 0.21.1 — справочник HTTP API
...
0.21.1. Patch не меняет endpoint, payload, status или permission contracts.
API предназначен для собственного браузерного UI и локальной
...
DCIM-данные (если включён collector), `information_system`, `dcim_project`,
`itsm`, `criticality_class` и единый текст Rooms/письма. Unicode-пробелы в
вставленном hostname удаляются до валидации и внешнего запроса. Resolver может
учитывать hostname, проект и информационную систему, но остаётся fail-closed
при отсутствии однозначного правила. Endpoint ничего не отправляет и не пишет
в БД, выполняется вне общего лока. Ошибки → 400.
