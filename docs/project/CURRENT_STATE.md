...
Дата проверки: 2026-07-16. Authoritative repository:
`~/Documents/prak_mai`.
...
Performance на disposable fixtures: 1k — 0.13 s, 10k — 1.28 s, 50k —
6.70 s. Рабочая `data/warehouse.db` не изменялась.

Финальный automated gate 0.14: 436 tests PASS (`skipped=8` для отсутствующих
...
Следующий приоритет — ручная операторская приёмка FULL Inventory 0.14,
target Equipment Query Port и отдельный controlled cutover design. До него
реальный initial-baseline publish запрещён. Затем — correction/reversal,
backup/restore drill и server-readiness. Monitoring, Reports и Wiki не должны
расширять Warehouse scope или блокировать его стабилизацию.
