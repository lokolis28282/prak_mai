...
Актуализирован по результатам полного gate (2026-07-25): назначение
каждого файла извлечено из его кода (docstring/структура), объёмы — фактические.

Сводные объёмы: **242 Python-файла** (~56 800 строк вместе с тестами),
**42 JavaScript-файла** (~4 700 строк), 8 лаунчеров, 574 автоматических теста.
...
| `webapp.py` | 2411 | HTTP-сервер (ThreadingHTTPServer): сессии (HttpOnly, SameSite=Strict), роутинг GET/POST, `/api/action` (все мутации под `service.lock`), валидация payload, CSV/XLSX download, раздача static с anti-traversal, security-заголовки. Известный монолит (HTML-шаблон внутри), разбирается постепенно. |
...
| Файл | Строк | Назначение |
|---|---|---|
| `facade.py` | 748 | `WarehouseFacade` — единственная публичная точка входа склада: обзор/KPI, баланс, карточки, приход/расход, кабели, поставки, инвентаризация, data-quality операции; posting-guard всех мутаций. |
...
| `issue_repository.py` | 330 | Персистентность расхода: FIFO-аллокации, проверки повторного списания и единый operation-level read-model для UI/полной выгрузки с целевой железкой. |
...
| `audit_repository_data.py` | 105 | Git gate: запрет runtime/company data artifacts и SQLite-содержимого в index. |
...
| `ui.js` | 878 | Баланс, справочники, «Контроль качества данных», сценарии прихода/расхода, последние операции с целевой железкой, lazy rendering тяжёлых таблиц, сканер-списки, поставки (частично), загрузка данных `loadAll()`, Timeline-подписи. |
| `product.js` | 1033 | Компонентный рендер: главная, карточки модулей, обзор склада (KPI + лента), Equipment Card, серверная догрузка баланса, поиск и миграционные review-экраны. |
...
`tests/headless_smoke.js` (259), `headless_migration_pilot_smoke.js` (94),
...
## 12. `tests/` — 73 Python test-файла, 574 теста
...
| Безопасность и платформа | `test_webapp_security` (226), `test_administration_api_contract`, `test_architecture`, `test_ui_navigation_architecture`, `test_stage_0_12_17`, `test_scanner_draft_frontend_contract`, `test_preview_limits`, `test_create_clean_test_db`, `test_repository_data_boundary`, `test_runtime_module_migration`, `test_warehouse_event_reader` | Сессии/роли/утечки traceback, границы модулей, Git data boundary, пустой bootstrap, навигация, лимиты preview, тестовая БД. |
