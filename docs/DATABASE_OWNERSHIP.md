...
Boundary introduced in ODE 0.12.6 and verified for source Stage 0.13.3A.5.
Module ownership is logical inside each file; Warehouse site isolation is
physical.

**CURRENT LOCAL FACT (2026-07-26):** `data/warehouse.db` is the IXcellerate
working database and validated promoted historical candidate.
`data/warehouse_solar.db` is a second Warehouse-only operational contour:
empty at first bootstrap except for a one-time copy of reference tables.
Reports, Monitoring, Knowledge and Administration continue to use the primary
application context. Vacations uses a third, standalone `data/vacations.db`.
Operational Warehouse rows and audit generated by Solar stay in the Solar DB.
Full contract:
...
| `vacation_employees` | Vacations | Vacations | VacationRepository via VacationFacade | no | stay Vacations |
| `vacation_assignments` | Vacations | Vacations | VacationRepository via VacationFacade | no | effective-dated site/schedule history |
| `vacation_requests` | Vacations | Vacations | VacationRepository via VacationFacade | no | stay Vacations; external Sfera integration is future |
| `vacation_conflicts` | Vacations | Vacations | VacationRepository via VacationFacade | no | explicit exception/rejection lifecycle |
| `vacation_history` | Vacations | Vacations | VacationRepository via VacationFacade | no | append-only module history |
| `vacation_audit_log` | Vacations | Vacations | VacationRepository via VacationFacade | no | standalone actor/action audit |
...
Vacations stores roster, effective assignments, requests, detected conflicts,
history and audit only in `data/vacations.db`. It never reads/writes either
Warehouse DB, the shared `audit_log`, or Reports/Monitoring/Knowledge-owned
data. Neither `data/warehouse.db` nor `data/warehouse_solar.db` may contain
`vacation_*` tables.
