...
- экран остатков использует один ленивый read-path
  `/api/warehouse-stock-tree`: `category → item_type → vendor → model`, ветви
  раскрываются и сворачиваются без дублирования строк, по 100 групп на
  страницу; старый плоский список и построчные action-кнопки не являются
  контрактом этого экрана;
- пользовательское имя экрана `balance` — `Баланс` во всех навигационных
  поверхностях;
- `Приход` и `Расход` постоянно показывают последние 20 проведённых операций
  вне scenario-stage; расход показывает целевое оборудование, S/N и hostname;
  полные выгрузки подписаны `Выгрузить все приходы/расходы` и не маскируются
  под выгрузку текущего Preview;
