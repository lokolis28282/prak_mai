...
Одна SQLite-БД. Один `WarehouseService`. Логика разделяется внутри сервисного слоя и таблиц по типу позиции.
...
- `inventory_number` обязателен для реальной инвентаризации.
- `quantity = 1`.
- `unit = 'шт'`.
...
- Большие импорты 100k+ строк требуют индексов до запуска на реальной базе.
...
- Импорт 100k строк укладывается в целевой лимит времени и памяти.
