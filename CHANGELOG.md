...
копирования всей папки и без замены локальных runtime-БД. Интегрированы:

- нормализация вставленного hostname с удалением Unicode-пробелов на frontend
  и backend до любого внешнего запроса;
- извлечение из DCIM проекта, информационной системы, ITSM и класса
  критичности, включая безопасный DOM fallback;
- маршрутизация по комбинации hostname/project/information system, learned
  Digital rules и `global_cc` при сохранении приоритета
  `Salt → Digital → X5Tech` и fail-closed неоднозначности;
- единый девятипунктовый шаблон Rooms/письма с приветствием по времени,
  локацией, S/N, ITSM и классом критичности;
- консервативный stdlib-генератор правил из подтверждённого XLSX и новые
  unit/frontend contracts.

Рабочие БД, локальные Monitoring JSON, отчёты анализа и одноразовый скрипт с
персональными Windows-путями не входят в Git. Для закрытого переноса может быть
