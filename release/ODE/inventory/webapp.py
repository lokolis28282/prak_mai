...
function renderWarehouseCategories(){document.getElementById('warehouseCategories').innerHTML=(state.warehouse_categories||[]).map((x,i)=>`<div class="card" style="border-top:4px solid ${['#2563eb','#0ea5e9','#8b5cf6','#14b8a6','#f59e0b','#64748b'][i]}"><span>${esc(x.name)}</span><strong>${Number(x.quantity).toLocaleString('ru-RU')}</strong></div>`).join('')}
...
document.getElementById('deliveryCsv').onchange=async e=>{const file=e.target.files[0];if(!file)return;try{const r=await request('/api/preview-csv?kind=delivery',{method:'POST',headers:{'Content-Type':'text/csv','X-Filename':encodeURIComponent(file.name)},body:file});const labels={'Ожидается':'Ожидается к приемке','Уже на складе':'Уже есть на складе','Дубль в файле':'Дубли в файле','Ошибка':'Ошибки'};document.getElementById('deliveryPreview').innerHTML=`<div class="box"><h3>Проверка файла</h3><div class="cards">${Object.entries(labels).map(([k,v])=>`<div class="card"><span>${v}</span><strong>${r.counts[k]||0}</strong></div>`).join('')}</div><div style="margin-top:12px"><button class="button primary" onclick="confirmDelivery('${r.preview_id}')">Загрузить поставку</button></div></div>`}catch(x){notify(x.message,true)}finally{e.target.value=''}};
...
const TYPE_VENDORS={
 'Сервер':['Dell','HPE','Lenovo','Huawei','XFusion','Supermicro','Другое'],
...
function startReceiptWizard(){
 ...
 stage.innerHTML='';stage.hidden=false;const data={};let step=0;
 const steps=[
  ...
  {title:'Укажите наименование',key:'item_name',values:()=>[...new Set((state.recent_receipts||[]).filter(x=>!data.model||x.model===data.model).map(x=>x.item_name).filter(Boolean))].slice(0,12),input:true},
 ...
 function render(){const s=steps[step];stage.innerHTML=`<div class="wizard-shell"><button class="wizard-back" type="button">← ${step?'Назад':'К способам приемки'}</button><div class="wizard-progress"><i style="width:${(step+1)/steps.length*100}%"></i></div><p>Шаг ${step+1} из ${steps.length}</p><h2>${s.title}</h2><div class="wizard-content"></div></div>`;stage.querySelector('.wizard-back').onclick=()=>{if(step){step--;render()}else root.showScenario&&root.showScenario('__none__')};const content=stage.querySelector('.wizard-content');
  if(s.common){content.innerHTML=`<div class="common-grid"><label>Проект<input id="wProject" list="ref-project"></label><label>Полка<input id="wShelf" list="ref-shelf" required autofocus></label><label>ЦОД<input id="wDc" value="Ixcellerate" required></label><label>Поставщик <small>необязательно</small><input id="wSupplier" list="ref-supplier"></label></div><button class="wizard-next" type="button">Перейти к сканированию</button>`;content.querySelector('button').onclick=()=>showScanner();return}
 ...
 function showScanner(){data.project=document.getElementById('wProject').value;data.shelf=document.getElementById('wShelf').value;data.datacenter=document.getElementById('wDc').value;data.supplier=document.getElementById('wSupplier').value||'Не указан';const f=document.getElementById('scanReceiptForm');Object.entries({receipt_date:today,responsible:document.querySelector('[name="responsible"]').value,supplier:data.supplier,vendor:data.vendor,model:data.model,item_name:data.item_name,project:data.project,datacenter:data.datacenter,shelf:data.shelf,object_name:'Склад',equipment_type:data.category==='Оборудование'?data.type:'',component_type:data.category==='Компоненты'?data.type:'',cable_type:'',unit:'шт'}).forEach(([k,v])=>{if(f.elements[k])f.elements[k].value=v});stage.innerHTML=`<div class="wizard-shell scan-step"><button class="wizard-back" type="button">← Параметры партии</button><p class="eyebrow">${data.type} · ${data.vendor} · ${data.model}</p><h2>Сканируйте S/N</h2><p>Каждый Enter добавляет позицию во временный список. В базе пока ничего не меняется.</p></div>`;stage.querySelector('.wizard-back').onclick=()=>{step=5;render()};stage.appendChild(scanner);scanner.hidden=false;f.hidden=true;document.getElementById('receiptScanner').focus()}
 render();
}
function rebuildScenarioCards(){const receipt=document.getElementById('receipt'),bar=receipt.querySelector('.scenario-cards');bar.innerHTML='';[['📷','Сканировать оборудование','Партия с серийными номерами',startReceiptWizard],['📁','Импорт поставки','Подготовить данные снабжения',()=>openTask('warehouse','deliveries')],['📦','Принять кабели','Быстро, без серийных номеров',()=>receipt.showScenario('Принять кабели')],['✍','Ручное добавление','Одна нестандартная позиция',()=>receipt.showScenario('Ручной ввод')]].forEach(([icon,title,help,fn])=>{const b=document.createElement('button');b.className='scenario-card';b.innerHTML=`<b>${icon}</b><strong>${title}</strong><span>${help}</span>`;b.onclick=fn;bar.appendChild(b)});const issue=document.getElementById('issue'),ib=issue.querySelector('.scenario-cards');ib.innerHTML='';[['📷','Сканировать оборудование','Найти по S/N и собрать список','Списать сканером'],['📋','Выдать из баланса','Выбрать из текущих остатков','Найти и списать из баланса'],['🧵','Списать кабели','Одна операция на всё количество','Списать кабели']].forEach(([icon,title,help,target])=>{const b=document.createElement('button');b.className='scenario-card';b.innerHTML=`<b>${icon}</b><strong>${title}</strong><span>${help}</span>`;b.onclick=()=>issue.showScenario(target);ib.appendChild(b)})}
function modernShell(){const top=document.querySelector('.top'),actions=top?.querySelector('.profile-actions');if(!top||!actions)return;const brand=top.querySelector('div:first-child');if(brand)brand.innerHTML='<div class="top-brand"><strong>ODE</strong><span>Отдел дежурных инженеров</span><span id="pageTitle" hidden>Главная</span></div>';const refresh=actions.querySelector('[onclick="loadAll()"]');if(!actions.querySelector('.shift-profile')){const profile=document.createElement('button');profile.className='button shift-profile';profile.textContent='Профиль';profile.onclick=openShiftProfile;actions.insertBefore(profile,refresh)}const logout=top.querySelector('[onclick="logout()"]');if(logout){logout.style.display='inline-block';logout.textContent='Сменить инженера'}const homeButton=document.querySelector('[data-section="home"]');if(homeButton)homeButton.textContent='Главная';}
...
USER_CSV_TEMPLATES = {
    ...
    "delivery": "Приходная ведомость;Дата заказа;Заявка;Заказ;Серийные номера;Номер поставки;Поставщик;Плановая дата поставки;Поз.Заявки;Поз.Заказа;Договор;PLU;Объект учета;шт;номер ОС;единица оборудования\r\n",
}
