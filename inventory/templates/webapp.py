...
<section class="view panel" id="daily"><h2>Отчет за смену</h2><div class="box"><h3>Новая запись</h3><p class="hint">Запись сохраняется в общий реестр работ и сразу видна во всех отчётах.</p><form class="form uvr-form" id="shiftLogForm" data-uvr-form><div class="uvr-form-grid"><div><label>Дата</label><input name="work_date" type="date" required></div><div><label>Источник задачи</label><select name="task_source" required></select></div><div><label>Номер задачи</label><input name="task_number" placeholder="14532" autocomplete="off"></div><div class="uvr-desc-cell"><label>Описание работ</label><input name="description" class="uvr-description-input" list="" autocomplete="off"><datalist class="uvr-description-options"></datalist></div><div><label>Раздел</label><select name="section"></select></div><div><label>Статус</label><select name="status" required></select></div><div><label>Срок выполнения</label><input name="due_date" type="date" required></div></div><div class="uvr-pnr-field" hidden><label>Выполненные работы (PNR)</label><div class="uvr-pnr-checklist"></div></div><label>Комментарий</label><textarea name="comment"></textarea><div class="actions"><button class="button primary">Добавить запись</button></div></form></div><div style="height:16px"></div><div class="report-tabs"><button class="report-tab active" type="button" data-report-tab="shift" onclick="reportsMode('shift')">За смену</button><button class="report-tab" type="button" data-report-tab="all" onclick="reportsMode('all')">Все работы</button></div><div class="report-pane" id="reportPaneShift"><div class="cards" id="shiftStats"></div><div style="height:12px"></div><div class="box"><h3>Выполненные работы за смену</h3><p class="hint">Работы за выбранный день. Инженер: <span id="shiftEngineer"></span></p><form class="filters uvr-report-form" id="shiftForm"><input name="date" type="date" required><button class="button primary">Сформировать</button><button class="button" type="button" id="exportShift">Экспорт в Excel</button></form></div><div style="height:12px"></div><div class="table-wrap"><table class="uvr-table"><thead><tr><th>Дата</th><th>Имя задачи</th><th>Описание работ</th><th>Статус</th><th>Раздел</th><th>Срок</th><th>Комментарий</th><th>Действия</th></tr></thead><tbody id="shiftBody"><tr><td class="empty" colspan="8">Выберите дату смены</td></tr></tbody></table></div></div><div class="report-pane" id="reportPaneAll" hidden><div class="import-box"><div><strong>Все работы (реестр)</strong><p>Полный список работ за всё время. Источник, тип и номер задачи хранятся отдельно.</p></div><div class="import-actions"><a class="button" href="/import/work-logs-template.csv">Шаблон CSV</a><a class="button" id="exportWorkLogs" href="/export/work-logs.xlsx">Выгрузить в Excel</a><label class="button primary" for="workLogsCsv">Импорт CSV</label><input class="file-input csv-input" id="workLogsCsv" data-kind="work_logs" type="file" accept=".csv"><label class="button" for="workLogsXlsx" title="Миграция данных из старого файла Баланс">Импорт из Excel</label><input class="file-input" id="workLogsXlsx" type="file" accept=".xlsx" style="display:none"></div></div><div class="filters uvr-toolbar"><input id="uvrSearch" type="search" placeholder="Поиск по всем полям" autocomplete="off"><input id="uvrFilterFrom" type="date" title="Дата с"><input id="uvrFilterTo" type="date" title="Дата по"><select id="uvrFilterStatus"><option value="">Все статусы</option></select><select id="uvrFilterSection"><option value="">Все разделы</option></select><label class="uvr-review-toggle"><input id="uvrReviewOnly" type="checkbox"> Только требует проверки</label><button class="button" type="button" onclick="clearUvrFilters()">Сбросить</button></div><div class="uvr-registry-info"><span id="uvrCount" class="hint"></span></div><div class="uvr-bulk" id="uvrBulk" hidden><span id="uvrBulkCount"></span><select id="uvrBulkSection"></select><button class="button small primary" type="button" onclick="uvrBulkAssign()">Назначить раздел выбранным</button><button class="button small" type="button" onclick="uvrClearSelection()">Снять выбор</button></div><div class="table-wrap"><table class="uvr-table"><thead><tr><th class="uvr-check-col"><input id="uvrSelectAll" type="checkbox" title="Выбрать всё"></th><th class="sortable" data-sort="work_date">Дата</th><th class="sortable" data-sort="full_task_name">Имя задачи</th><th class="sortable" data-sort="description">Описание работ</th><th class="sortable" data-sort="status">Статус</th><th class="sortable" data-sort="section">Раздел</th><th class="sortable" data-sort="due_date">Срок</th><th>Комментарий</th><th>Действия</th></tr></thead><tbody id="workLogBody"></tbody></table></div><div class="uvr-pager" id="uvrPager"></div><details class="uvr-legacy"><summary>Складская детализация и загруженные CSV-отчеты</summary><div class="box" style="margin-top:12px"><h3>Детализация из базы (логи, приход, расход)</h3><form class="filters" id="dailyForm"><input name="date_from" type="date" required><input name="date_to" type="date" required><button class="button primary">Сформировать отчет</button><button class="button" type="button" id="downloadDaily">Скачать в Excel</button></form></div><div class="box" style="margin-top:12px"><h3>Загрузить готовый CSV отчет</h3><div class="import-actions"><a class="button" href="/import/daily-report-template.csv">Шаблон</a><label class="button primary" for="dailyCsv">Загрузить CSV</label><input class="file-input csv-input" id="dailyCsv" data-kind="daily_report" type="file" accept=".csv"><select id="uploadedReport"></select><button class="button" onclick="showUploadedReport()">Показать</button><button class="button" onclick="exportUploadedReport()">Экспорт</button></div></div><div class="table-wrap" style="margin-top:12px"><table><thead><tr><th>Дата</th><th>Блок</th><th>Номер задачи</th><th>Описание / наименование</th><th>Кол-во / метраж</th><th>S/N</th><th>ФИО</th><th>Комментарий / основание</th></tr></thead><tbody id="dailyBody"><tr><td class="empty" colspan="8">Выберите источник отчета</td></tr></tbody></table></div></details></div></section><section class="view panel" id="handover"><h2>Передача по смене</h2><div class="box"><h3>Незавершённые задачи</h3><p class="hint">Автоматически: все задачи со статусом кроме «Выполнено». Полностью выполненные (в т.ч. PNR со всеми галками) не передаются — задачи не теряются между сменами.</p><div class="filters uvr-toolbar"><input id="handoverSearch" type="search" placeholder="Поиск по всем полям" autocomplete="off"><input id="handoverFrom" type="date" title="Дата с"><input id="handoverTo" type="date" title="Дата по"><select id="handoverStatus"><option value="">Все незавершённые</option></select><button class="button" type="button" onclick="clearHandoverFilters()">Сбросить</button><a class="button" id="handoverExport" href="/export/work-logs.xlsx">Выгрузить в Excel</a></div></div><div style="height:12px"></div><div class="table-wrap"><table class="uvr-table"><thead><tr><th class="sortable" data-sort="work_date">Дата</th><th class="sortable" data-sort="task_source">Источник</th><th class="sortable" data-sort="task_number">Номер</th><th class="sortable" data-sort="description">Описание</th><th class="sortable" data-sort="status">Статус</th><th class="sortable" data-sort="due_date">Срок</th><th>Комментарий</th><th>Действия</th></tr></thead><tbody id="handoverBody"><tr><td class="empty" colspan="8">Загрузка…</td></tr></tbody></table></div></section>



<section class="view panel" id="shipments"><h2>Учет поставок-отправок</h2><div class="placeholder">В разработке. Здесь будет учет взаимодействия со снабжением, поставками, отправками и будущая выгрузка/внесение данных в DCIM</div></section><section class="view panel" id="profile"><h2>Профиль</h2><div class="split"><div class="box"><h3>Личные данные</h3><form class="form" id="profileForm"><label>Имя</label><input name="first_name" required><label>Фамилия</label><input name="last_name" required><label>Должность</label><input name="position" required><label>Email</label><input name="email" type="email" readonly><div class="actions"><button class="button primary">Сохранить</button></div></form></div><div class="box"><h3>Смена пароля</h3><form class="form" id="passwordForm"><label>Текущий пароль</label><input name="old_password" type="password" autocomplete="current-password" required><label>Новый пароль</label><input name="new_password" type="password" autocomplete="new-password" minlength="6" required><div class="actions"><button class="button primary">Сменить пароль</button></div></form></div></div></section><section class="view panel" id="kaiten"><h2>Kaiten</h2><div class="placeholder">Интеграция будет реализована позднее.</div></section><section class="view panel" id="weekly"><h2>Отчет за неделю</h2><div class="box"><h3>Выполненные работы за период</h3><p class="hint">Выберите период — соберутся все работы из УВР. Инженер: <span id="weekEngineer"></span></p><form class="filters uvr-report-form" id="weekReportForm"><input name="date_from" type="date" required><input name="date_to" type="date" required><button class="button primary">Сформировать</button><button class="button" type="button" id="exportWeek">Выгрузить в Excel</button></form></div><div style="height:16px"></div><div class="table-wrap"><table class="uvr-table"><thead><tr><th>Дата</th><th>Имя задачи</th><th>Описание работ</th><th>Статус</th><th>Раздел</th><th>Срок</th><th>Комментарий</th></tr></thead><tbody id="weekBody"><tr><td class="empty" colspan="7">Выберите период</td></tr></tbody></table></div><details class="uvr-legacy"><summary>Складская агрегация за период</summary><div class="box" style="margin-top:12px"><p class="hint">Агрегация существующих логов, приходов и расходов.</p><form class="filters" id="weeklyForm"><input name="date_from" type="date" required><input name="date_to" type="date" required><button class="button primary">Сформировать</button><button class="button" type="button" id="downloadWeekly">Выгрузить в Excel</button></form><div class="cards" id="weeklyCards"></div><div class="split" style="margin-top:18px"><div class="box"><h3>По проектам</h3><div class="table-wrap"><table><thead><tr><th>Проект</th><th>Принято</th><th>Списано</th></tr></thead><tbody id="weeklyProjects"></tbody></table></div></div><div class="box"><h3>По типам</h3><div class="table-wrap"><table><thead><tr><th>Тип</th><th>Принято</th><th>Списано</th></tr></thead><tbody id="weeklyTypes"></tbody></table></div></div></div></div></details></section><section class="view panel" id="monitoring"><h2>Мониторинг __DATACENTER__</h2><div class="placeholder">В разработке</div></section>
...
HTML = HTML.replace(
    ...
    'id="exportWorkLogs" href="/export/work-logs.xlsx">Выгрузить в Excel</a>', 'id="exportWorkLogs" href="/export/work-logs.xlsx">Скачать в Excel</a>',
).replace(
    'id="downloadWeekly">Выгрузить в Excel</button>', 'id="downloadWeekly">Скачать в Excel</button>',
).replace(
...
def _externalized_html(html: str) -> str:
    ...
    script_tags = "".join(
        ...
        for name in (
            ...
            "reports/index.js", "reports/form.js", "reports/work_logs.js",
            "reports/daily.js", "reports/handover.js", "reports/weekly.js",
            "monitoring/index.js",
            "knowledge/index.js",
