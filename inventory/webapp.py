...
HOME_SECTION = r'''<section class="view panel home-screen" id="home"><h2>Что нужно сделать?</h2><div class="home-actions">
<button class="home-action primary" onclick="openTask('warehouse','receipt')"><strong>Принять на склад</strong><span>Сканировать серийный номер или загрузить поставку</span></button>
<button class="home-action" onclick="openTask('warehouse','issue')"><strong>Списать со склада</strong><span>Сканировать серийный номер оборудования</span></button>
<button class="home-action" onclick="openTask('warehouse','balance')"><strong>Найти позицию</strong><span>По серийному номеру, модели или полке</span></button>
<button class="home-action" onclick="openTask('warehouse','inventory')"><strong>Проверить инвентаризацию</strong><span>Сверить список с остатками</span></button>
<button class="home-action" onclick="openTask('reports','daily')"><strong>Сделать отчет</strong><span>Ежедневный, еженедельный или журнал работ</span></button>
</div></section>'''
...
HTML = HTML.replace(
    ...
    "function showSection(name){currentSection=name;document.getElementById('pageTitle').textContent={home:'Главная',warehouse:'Склад',reports:'Отчеты',administration:'Администрирование',monitoring:'Мониторинг',profile:'Профиль'}[name];",
).replace(
    "nav.style.display='flex';nav.innerHTML=sections[name].map",
    "nav.style.display=['home','monitoring','profile'].includes(name)?'none':'flex';nav.innerHTML=sections[name].map",
).replace(
    ...
    "function showProfile(){document.querySelectorAll('.section-button').forEach(x=>x.classList.remove('active'));document.getElementById('subnav').style.display='none';showView('profile')}",
    "function showProfile(){document.querySelectorAll('.section-button').forEach(x=>x.classList.remove('active'));document.getElementById('pageTitle').textContent='Профиль';document.getElementById('subnav').style.display='none';showView('profile')}",
).replace(
...
HTML = HTML.replace('<body>', '<body data-ui-labels="Склад Ixcellerate | Скачать баланс | Предпросмотр файла | Создать резервную копию | Загрузить базу">', 1)
