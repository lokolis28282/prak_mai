...
def make_handler(application: WarehouseService | ApplicationContext) -> type[BaseHTTPRequestHandler]:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _do_POST(self) -> None:
            ...
            try:
                ...
                if action in {"RECEIPT", "ISSUE"}:
                    ...
                    app_context.reports.create_work_log(self._work_log_payload(data))
                elif action == "UPDATE_WORK_LOG":
                    app_context.reports.update_work_log(
                        self._query_int_value(data.get("id"), "id"),
                        self._work_log_payload(data),
                    )
                elif action == "DELETE_WORK_LOG":
                    app_context.reports.delete_work_log(
                        self._query_int_value(data.get("id"), "id")
                    )
        ...
        def _preview_work_log_xlsx(self, sheet: str) -> None:
            try:
                length = int(self.headers.get("Content-Length", "0"))
                if length <= 0:
                ...
                body = self.rfile.read(length)
                result = app_context.reports.preview_work_log_xlsx(
                    body,
                    sheet_name=sheet,
                    filename=unquote(self.headers.get("X-Filename", "work_logs.xlsx")),
                )
                self._send_json(200, {"ok": True, **result})
        ...
        def _validate_action_payload(data: dict[str, Any]) -> None:
            ...
            numeric_fields = {"equipment_id", "quantity", "delivery_id", "reference_id", "id"}
