...
def handle_get(handler: Any, runtime: RouteRuntime, path: str, query: dict[str, list[str]]):
    ...
    if path == '/api/data':
        ...
        self._send_json(200, {**warehouse_data, 'task_sources': list(service.TASK_SOURCES), 'task_types': list(service.TASK_TYPES), 'work_log_statuses': list(service.WORK_LOG_STATUSES), 'report_references': app_context.reports.reference_options(), 'daily_report_uploads': app_context.reports.daily_report_uploads(), 'current_user': current_user, 'warehouse_site': {'key': runtime.warehouse_key, 'label': runtime.warehouse_label}, 'runtime': {'database': service.db_path.name, 'database_fingerprint': database_fingerprint, 'working_database': not migration_full_status.get('read_only') and (not migration_pilot_status.get('enabled'))}, 'migration_pilot': migration_pilot_status, 'migration_full': migration_full_status, 'warehouse_system': app_context.warehouse.get_system_status()})
