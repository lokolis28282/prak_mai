-- ODE external Preview workspace schema v2. Never apply to operational DB.
...
CREATE INDEX ix_preview_runs_session ON preview_runs(session_id, attempt, run_id);
CREATE UNIQUE INDEX ux_preview_ready_digest ON preview_runs(session_id, preview_digest)
...
CREATE INDEX ix_preview_rows_status_page ON preview_rows(run_id, row_status, row_id);
CREATE INDEX ix_preview_rows_match ON preview_rows(run_id, proposed_match_key, row_id);
...
CREATE TABLE preview_findings (
    ...
    finding_status TEXT NOT NULL CHECK (finding_status IN ('OPEN', 'RESOLVED', 'WAIVED')),
...
CREATE INDEX ix_preview_matches_row ON preview_matches(run_id, row_id, match_kind, match_id);
...
CREATE TABLE preview_resolutions (
    ...
    supersedes_resolution_id INTEGER UNIQUE REFERENCES preview_resolutions(resolution_id),
...
CREATE INDEX ix_preview_resolutions_row ON preview_resolutions(run_id, row_id, resolution_id);
