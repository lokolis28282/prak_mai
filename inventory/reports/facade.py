...
class ReportsFacade:
    ...
    def reference_options(self) -> dict[str, list[str]]:
        return _plain(self.repository.reference_options())
    ...
    def list_work_logs(self, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        return self.work_logs(
            ...
            section=query["section"], needs_review=query["needs_review"],
        )
    ...
    def assign_section(self, log_ids: list[int], section: str) -> int:
        ...
        return int(self.work_log_service.assign_section(log_ids, section))
    ...
    def handover_logs(self, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        requested_status = str(query.pop("status", "") or "").strip()
        rows = self.list_work_logs(query)
        pending = [
            row for row in rows
            if str(row.get("status", "")).strip() != STATUS_DONE
            and (
                not requested_status
                or str(row.get("status", "")).strip() == requested_status
            )
        ]
        for row in pending:
    ...
    def shift_report_xlsx(self, report_date: str) -> bytes:
        ...
        done_rows = [
            row for row in self.list_work_logs(day)
            if str(row.get("status", "")).strip() == "Выполнено"
        ]
        # A handover must include older unfinished work as well as tasks opened
        # during the selected shift. Future-dated rows do not belong to it.
        handover_rows = self.handover_logs({"date_to": report_date})
    ...
    def work_logs_xlsx(self, filters: dict[str, Any] | None = None) -> bytes:
        return self._single_sheet_xlsx(
            "Все работы", "Все работы", WORK_LOG_EXPORT_COLUMNS,
            self.export_work_logs_rows(filters),
        )

    def daily_report_xlsx(self, report_date: str) -> bytes:
        return self._single_sheet_xlsx(
            "Отчет за смену", "Отчет за смену", DAILY_REPORT_EXPORT_COLUMNS,
            self.export_daily_report_rows(report_date),
        )

    def uploaded_report_xlsx(self, upload_id: int) -> bytes:
        return self._single_sheet_xlsx(
            "Загруженный отчет", "Загруженный отчет", DAILY_REPORT_EXPORT_COLUMNS,
            self.export_uploaded_report_rows(upload_id),
        )

    def handover_xlsx(self, filters: dict[str, Any] | None = None) -> bytes:
        columns = (
            ("work_date", "Дата"),
        ...
        return self._single_sheet_xlsx(
            "Передача по смене", "Передача по смене", columns,
            self.handover_logs(filters),
        )
