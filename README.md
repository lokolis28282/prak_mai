# ODE 0.12.17 RC1 — учет работ и склада

Release Candidate для контролируемой локальной эксплуатации. В версии 0.12.17
добавлены Dashboard, постоянный глобальный поиск, расширенная карточка
оборудования с историей, складские проблемы и события, ограниченные выдачи для
больших баз и усиленная серверная граница ролей. Приход, расход, кабели,
поставки, инвентаризация и отчеты сохраняют совместимые сценарии и API.
...
После входа открывается Dashboard с остатками, сегодняшними приходами и расходами,
проблемами, поставками, последними действиями и быстрыми командами. Поиск в
верхней части доступен на каждом экране и ищет S/N, инвентарный номер, hostname,
наименование, тип, вендора, модель, поставку, заказ, проект, полку, ЦОД и инженера.
Кнопка `ODE` всегда возвращает на Главную. Основная работа выполняется в разделе
`Склад`:
...
Текущий набор содержит 185 автоматических тестов, включая CSV и шаблоны, сканирование, поставки, карточки, глобальный поиск, массовые операции, rollback, отчеты, справочники, баланс, пользователей, роли, сессии, безопасность, резервные копии и аудит.
