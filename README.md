...
`python3 app.py`; он использует `data/warehouse.db`. Monitoring и Reports
сохранены на главной как карточки, но открывают только страницу «В разработке»
и не используют складские workflow. У Monitoring уже реализован изолированный
backend hostname routing; operator UI, внешние collectors и отправка писем пока
не подключены.
...
- Monitoring UI, collectors и отправка писем пока не реализованы; hostname
  routing backend готов. Kaiten остаётся заглушкой, недельный отчёт — базовой
  агрегацией без отдельного конструктора;
