...
> macOS / Linux · внешние зависимости не требуются · 685 автоматических тестов
...
- **УВР и отчёты** — общий реестр работ, PNR-чек-лист, передача незавершённых
  задач, XLSX/CSV-импорт и форматированные XLSX-отчёты за смену и неделю
  (отдельный контур, не пишет в складские таблицы).
...
Раздел `Отчеты` содержит УВР с поиском, серверными фильтрами, страницами по 25
строк, CRUD, PNR-чек-листом, обязательным сроком и Preview → Confirm импортом.
`Передача по смене` выводит только незавершённые задачи и конкретные оставшиеся
PNR-действия; XLSX смены разделяет выполненные работы и backlog на два листа.
Экран `Мониторинг`
...
| `work_logs` | УВР: дата, источник/номер, описание, статус, раздел, срок, PNR-чек-лист, review и комментарий; legacy `task_type` сохранён для импорта |
...
Текущий discover-набор версии 0.20.0 содержит 685 автоматических тестов
...
Focused UX-проверка Reports на одноразовой копии:

```bash
python3 scripts/smoke_ui.py --headless-script tests/headless_reports_smoke.js --headless-arg engineer
python3 scripts/smoke_ui.py --headless-script tests/headless_reports_smoke.js --headless-arg viewer --login-role viewer
```
