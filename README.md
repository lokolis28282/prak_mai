# ODE — source Stage 0.13.2, runtime metadata 0.12.17.1 RC2

Текущий исходный код соответствует Stage 0.13.2. Он добавляет назначение
Inventory Number в существующей карточке и массовый CSV workflow
`Preview -> Confirm -> Result`, сохраняя S/N главным идентификатором. Runtime-
метаданные исходников и target текущего package builder остаются
`0.12.17.1 RC2`, но последний фактически собранный Windows ZIP содержит
`ODE 0.12.17 RC1`. ZIP для RC2 и Stage 0.13.2 не создавался; перед следующим
Windows-релизом metadata, builder и release notes должны быть синхронизированы
отдельным release change.
...
В разделе `Поставки` загружается документ снабжения, проверяются дубли и уже принятые S/N, заполняются недостающие поля и выполняется приемка сканером. Раздел `Отчеты` содержит три вкладки с одинаковыми столбцами (Дата, Имя задачи, Описание работ, Статус, Раздел, Тип, Комментарий): `УВР` — полный реестр выполненных работ за всё время с поиском, сортировкой, фильтрами, добавлением, редактированием и удалением записей и разовым импортом истории из старого файла Excel; `Отчет за смену` — таблица работ за выбранную дату; `Отчет за неделю` — таблица работ за период. Оба отчета показывают инженера и выгружаются в CSV для Excel именно как список работ за период. Раздел выбирается из строгого списка; новые записи ведутся по нему, а старые импортированные значения сохраняются. `Мониторинг` содержит только явную заглушку «В разработке»; складские проблемы и события из него удалены. Kaiten и внешние интеграции в текущую версию не входят.
...
2. Используйте только заранее проверенный архив с явно указанной версией.
   Текущий source Stage 0.13.2 нельзя собирать/публиковать старым
...
Текущий набор содержит 246 автоматических тестов, включая CSV и шаблоны,
сканирование, поставки, карточки, глобальный поиск, массовое назначение
Inventory Number, УВР (CRUD логов работ, XLSX-импорт, текстовые отчеты смены и
недели), атомарный rollback, отчеты, справочники, баланс,
пользователей, роли, сессии, безопасность, резервные копии и аудит.
...
- Stage 0.13.2 ещё не собран в отдельный Windows ZIP и требует пробного импорта
  на disposable test DB перед работой с реальными данными;
