# ODE 0.12.16 RC1 — учет работ и склада

Release Candidate для тестовой эксплуатации: складские приход и расход
оборудования/компонентов, отдельный учет кабелей, импорт документа поставки и
фактическая приемка поставки проверены через `WarehouseFacade`. Приемка
поставки создает складской приход только после явного подтверждения, existing
S/N не дублируется, конфликтные заполненные поля не перезаписываются.
...
[ARCHITECTURE.md](ARCHITECTURE.md) и [индекс технической документации](docs/README.md).
