...
Главный операционный контур — **Склад**. Обычный запуск — `python3 app.py`;
он использует `data/warehouse.db`. Monitoring теперь имеет
...
Reports остаётся отдельным контуром и предоставляет УВР, отчёты за смену и
неделю, CSV/XLSX import/export без прямой записи в складские таблицы.
...
![ODE 0.14 architecture graph](docs/assets/ode-architecture-graph.svg)
...
Полный discover-набор содержит автоматические проверки Warehouse, Monitoring,
Knowledge и УВР. Исторические
