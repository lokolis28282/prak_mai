...
<script>
let sections={warehouse:[['overview','Обзор'],['receipt','Приход'],['issue','Расход'],['balance','Баланс'],['equipment','Оборудование'],['journal','Журнал'],['shipments','Учет поставок-отправок'],['references','Справочники'],['admin','Администрирование'],['instruction','Инструкция']],reports:[['daily','Ежедневные отчеты'],['worklogs','Логи работ'],['kaiten','Kaiten'],['weekly','Еженедельный отчет']],monitoring:[['monitoring','В разработке']]};
let state={equipment:[],operations:[],categories:[],locations:[],stats:{},task_sources:[],task_types:[],work_log_statuses:[],references:[],reference_kinds:{},stock_receipts:[],stock_issues:[],balance:[],searchRows:[],daily_report_uploads:[],current_user:{}};let currentSection='warehouse';
...
const balanceFilterMap={balanceProject:'project',balanceObject:'object_name',balanceEquipmentType:'equipment_type',balanceComponentType:'component_type',balanceCableType:'cable_type',balanceUnit:'unit',balanceDatacenter:'datacenter'};function activeBalanceFilters(){return Object.fromEntries(Object.entries(balanceFilterMap).map(([id,key])=>[key,document.getElementById(id).value]).filter(x=>x[1]))}function rowMatchesQuery(x,q){return !q||['serial_number','inventory_number','item_name','model','vendor','project','object_name','shelf'].some(k=>String(x[k]||'').toLocaleLowerCase().includes(q))}function renderBalance(){const filters=activeBalanceFilters();const query=document.getElementById('balanceQuery').value.trim().toLocaleLowerCase();const rows=state.balance.filter(x=>Object.entries(filters).every(([k,v])=>x[k]===v)&&rowMatchesQuery(x,query));document.getElementById('balanceBody').innerHTML=rows.map(x=>{const key=encodeURIComponent(x.position_key);return `<tr><td>${esc(x.project)}</td><td>${esc(x.item_name)}</td><td>${esc(x.vendor)}</td><td>${esc(x.model)}</td><td>${esc(x.serial_number)}</td><td>${esc(x.inventory_number)}</td><td>${Number(x.balance).toLocaleString('ru-RU')}</td><td>${esc(x.unit)}</td><td>${esc(x.shelf)}</td><td>${esc(x.object_name)}</td><td>${esc(x.equipment_type)}</td><td>${esc(x.component_type)}</td><td>${esc(x.cable_type)}</td><td>${esc(x.datacenter)}</td><td><button class="button" onclick="openPositionCard('${key}')">Открыть</button> <button class="button" ${Number(x.balance)<=0?'disabled':''} onclick="selectForIssue('${key}')">Списать</button></td></tr>`}).join('')||'<tr><td class="empty" colspan="15">Нет данных</td></tr>';document.getElementById('balanceExport').href='/export/balance.csv?'+new URLSearchParams({...filters,query:document.getElementById('balanceQuery').value.trim()})}
...
async function loadAll(){try{state=await request('/api/data');state.searchRows=[];for(const [key,id] of [['receipts','statReceipts'],['issues','statIssues'],['balance','statBalance'],['positions','statPositions']])document.getElementById(id).textContent=Number(state.stats[key]).toLocaleString('ru-RU');document.getElementById('currentUser').textContent=`${state.current_user.first_name} ${state.current_user.last_name} · ${state.current_user.role}`;for(const name of ['first_name','last_name','position','email'])document.querySelector(`#profileForm [name=${name}]`).value=state.current_user[name]||'';if(state.current_user.role!=='admin'){sections.warehouse=sections.warehouse.filter(x=>x[0]!=='admin');if(document.querySelector('[data-view=admin]'))showSection(currentSection)}if(state.current_user.role==='viewer'){for(const id of ['stockReceiptForm','stockIssueForm','bulkIssueForm','addForm','moveForm','workLogForm','referenceForm']){const x=document.getElementById(id);if(x)x.style.display='none'}document.querySelectorAll('.csv-input,.preview-input').forEach(x=>x.closest('.import-actions')?.querySelector('label')?.remove())}fillSelects();renderBalance();renderOperations();if(state.current_user.must_change_password)notify('Рекомендуется сменить пароль по умолчанию в разделе «Профиль»')}catch(e){notify(e.message,true)}}
...
def make_handler(service: WarehouseService) -> type[BaseHTTPRequestHandler]:
    ...
    class Handler(BaseHTTPRequestHandler):
            ...
            try:
                if path == "/":
                    ...
                    self._send_template(
                        ...
                        "Hostname целевого оборудования,Кол-во,"
                        "S/N списываемого,Наименование,Тип кабеля,Комментарий\r\n",
                    )
                elif path == "/import/bulk-issue-template.csv":
                    self._send_template("bulk_issue_template.csv", "S/N,Комментарий\r\n")
        ...
        def _import_csv(self, kind: str, preview: bool = False) -> None:
            try:
                if kind not in {
                    "equipment", "receipt", "issue", "bulk_issue", "work_logs", "daily_report"
                }:
                ...
                if kind == "equipment":
                    ...
                    if preview:
                        result = service.preview_stock_receipt_rows(rows)
                        self._send_json(200, {"ok": True, **result})
                        return
                    ...
                    if preview:
                        result = service.preview_stock_issue_rows(rows)
                        self._send_json(200, {"ok": True, **result})
                        return
