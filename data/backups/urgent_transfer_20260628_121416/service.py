...
class WarehouseService:
    ...
    def _store_import_preview(
        ...
        while len(self._import_previews) > 20:
    ...
    def preview_stock_receipt_rows(
        ...
        with connect(self.db_path) as db:
            ...
            for line, source in enumerate(source_rows, start=2):
                ...
                try:
                    prepared = self._prepare_receipt(source, references, line)
        ...
        return self._store_import_preview("receipt", source_rows, {
            ...
            "duplicates": duplicates, "error_count": len(errors),
    ...
    def confirm_stock_receipt_preview(self, preview_id: str) -> int:
        ...
        check = self.preview_stock_receipt_rows(preview["rows"])
        self._import_previews.pop(check["preview_id"], None)
        if check["errors"]:
            raise WarehouseError(check["errors"][0]["reason"])
        imported = self.import_stock_receipt_rows(preview["rows"])
    ...
    def position_card(
        ...
        for row in issues:
            history.append({
                "date": row["issue_date"], "event_type": "Расход / аллокация",
        ...
        for row in audits:
            history.append({
                "date": row["event_date"], "event_type": f"Аудит: {row['action']}",
        ...
        with connect(self.db_path) as db:
            ...
            try:
                for line, source in enumerate(source_rows, start=2):
                    ...
                    try:
                        prepared = self._prepare_issue(source, references, line)
    ...
    def confirm_stock_issue_preview(self, preview_id: str) -> int:
        ...
        check = self.preview_stock_issue_rows(preview["rows"])
        self._import_previews.pop(check["preview_id"], None)
        if check["errors"]:
            raise WarehouseError(check["errors"][0]["reason"])
        imported = self.import_stock_issue_rows(preview["rows"])
        self._import_previews.pop(preview_id, None)
        return imported
