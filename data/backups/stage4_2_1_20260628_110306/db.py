...
CREATE TABLE IF NOT EXISTS stock_receipts (
    ...
    unit TEXT NOT NULL CHECK (unit IN ('шт', 'м')),
...
def initialize(db_path: str | Path = DEFAULT_DB_PATH) -> bool:
    ...
    with connect(db_path) as connection:
        ...
        references = {
            ...
            "task_source": ("DCIM", "ITSM", "Outlook", "Zabbix", "Складские операции"),
            "task_type": ("ИЗМ", "ПНР", "ЗНР", "ЗНО", "ИНЦ"),
            "work_log_status": ("Выполнено", "В работе", "В ожидании"),
            "supplier": ("Не указан",),
            "vendor": ("Не указан",),
            "unit": ("шт", "м"),
        }
